...
export default class ClashOfClans extends BaseGame {
  ...
  constructor(client: Client) {
    ...
    this.clans = {
      leagueGroup: (tag: string): Promise<ClashOfClansLeagueGroup> =>
        this.leagueGroup(tag),
      leagueWar: (tag: string): Promise<ClashOfClansLeagueWar> =>
        this.leagueWar(tag),
      warlog: (tag: string): Promise<Array<ClashOfClansWarlog>> =>
        this.warlog(tag),
      find: (name: string): Promise<Array<ClashOfClansBaseClan>> =>
        this.find(name),
      currentWar: (tag: string): Promise<ClashOfClansCurrentWar> =>
        this.currentWar(tag),
      get: (
        tag: string,
      ): Promise<ClashOfClansModifiedClan | ClashOfClansBaseClan> =>
        this.clan(tag),
      members: (tag: string): Promise<Array<ClashOfClansMember>> =>
        this.members(tag),
      capitalRaidSeasons: (
        tag: string,
      ): Promise<Array<ClashOfClansCapitalRaidSeason>> =>
        this.capitalRaidSeasons(tag),
    };

    this.players = {
      find: (tag: string) => this.player(tag),
    };
  ...
  private async leagueGroup<T>(tag: string): Promise<T> {
    return await this.cocFetch("clans", {
      tag,
      subdomain: "currentwar/leaguegroup",
    });
  }
  ...
  private async warlog(tag: string) {
    const response: Record<
      "items",
      Array<ClashOfClansWarlog>
    > = await this.cocFetch("clans", {
      tag,
      subdomain: "warlog",
    });
  ...
  private async find(name: string) {
    ...
    const response: Record<
      "items",
      Array<ClashOfClansBaseClan>
    > = await this.cocFetch("clans", { name });
   ...
   * @returns something
   */
  private async members<T extends Record<string, Array<ClashOfClansMember>>>(
    tag: string,
  ) {
    const response: T = await this.cocFetch("clans", {
      tag,
      subdomain: "members",
    });
   ...
   * @returns something
   */
  private async capitalRaidSeasons<
    T extends Record<string, Array<ClashOfClansCapitalRaidSeason>>,
  >(tag: string) {
    const response: T = await this.cocFetch("clans", {
      tag,
      subdomain: "capitalraidseasons",
    });
  ...
  private async modifyClan(
    clan: ClashOfClansBaseClan,
    baseClan: boolean = false,
  ): Promise<ClashOfClansModifiedClan> {
    const tag = clan.tag;
